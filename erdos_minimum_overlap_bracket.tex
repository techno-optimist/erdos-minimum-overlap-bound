...
takes responsibility for all claims. The upper-bound \emph{construction} is
due to the agent ``Hyra'' (EinsteinArena, problem \texttt{erdos-min-overlap});
the lower-bound \emph{method} is due to E.~P.~White \cite{White22}. See the
...
\[
0.379005 \;\le\; \mu \;\le\; Q \;<\; 0.3808669097979875909124431,
\]
where $Q$ is an explicit rational number, evaluated in exact integer
...
by $5.99\times10^{-5}$, and lies below the floating-point records reported
by recent AI-search systems \cite{AlphaEvolve25,TTT26,SimpleTES26}. It is
obtained by evaluating, exactly, the overlap
functional of an explicit $n=2400$ step-function construction found by the
AI search agent ``Hyra'' on the EinsteinArena platform; a short
piecewise-linearity lemma reduces the continuum supremum to a finite exact
computation. The lower bound is Theorem~1 of White \cite{White22}, quoted
unchanged. On the lower-bound side we additionally report a computer-assisted
...
EinsteinArena platform \cite{Hyra26} has produced the $n = 2400$
construction certified here. These reports are numerical scores of
...
rational $Q < 0.3808669097979875909124431$, a reduction of the bracket width
by about $3.1\%$. The upper bound is theorem-grade in the following concrete
...
a finite exact-integer-arithmetic computation that is fully reproducible from
...
The step-function \emph{construction} behind the new upper bound was produced
by the agent ``Hyra'' --- an anonymous AI search agent competing on the
EinsteinArena platform (problem \texttt{erdos-min-overlap}) \cite{Hyra26},
within an ecosystem of AI agents doing open iterative optimization on this
problem; our contribution on the upper side is limited to the
piecewise-linearity lemma and the exact rational evaluation. The lower-bound
...
The candidate is a vector $f = (f_1, \dots, f_n) \in [0,1]^n$ with $n = 2400$,
found by Hyra \cite{Hyra26}. The EinsteinArena board scores such a vector by
the following floating-point semantics: rescale $f \leftarrow f \cdot
\frac{n/2}{\sum_i f_i}$, set $g = 1 - f$, and report
...
is $0.3808669097979876$. To convert this into a bound on $\mu$ two things are
needed: (i)~a proof that the \emph{discrete} maximum over lattice lags equals
the \emph{continuum} supremum $\sup_x M(x)$ for the associated step function,
and (ii)~an evaluation of that maximum in exact arithmetic, including the
rescaling. We do both. Figure~\ref{fig:construction} shows the
construction and its overlap function.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{fig_hyra_construction}
\caption{The certified construction. \emph{Top:} the exactly rescaled Hyra
step function $F$ on $[-1,1]$ ($n = 2400$ cells). \emph{Bottom:} its
...
through the $4801$ lattice values $M(mh)$; the dotted line is the mean
value $\tfrac14$ of $M$ on $[-2,2]$. The maximum is attained at
$x^\ast = -184/2400$, where $M(x^\ast) = Q$. Display is floating point;
...
\begin{theorem}[upper bound; construction: Hyra \cite{Hyra26}]
\label{thm:upper}
Let $\mu$ be the Erd\H{o}s minimum overlap constant. Then
\[
\mu \;\le\; Q \;:=\;
\frac{1424992289798782609633201801352767458976314440679252577}
     {3741444197802851304404516484910431627947663875649308401},
\]
and
\[
0.3808669097979875909124430 \;\le\; Q \;<\; 0.3808669097979875909124431 .
\]
In particular $\mu \le 0.3808669097979875909124431 <
0.3809268534330870$, improving \eqref{eq:haugland}.
\end{theorem}

\begin{proof}
The raw Hyra vector $v \in [0,1]^{2400}$ is a list of IEEE-754 doubles; each
double is an exact dyadic rational, and the conversion
$\mathrm{Fraction}(v_i)$ is exact. Over the common denominator $2^{82}$ the
raw sum is
\[
\sum_i v_i \;=\; \frac{5802843938985922938555908853}{4835703278458516698824704}
\;=\; 1200 + 9.999999\ldots\times10^{-7},
\]
so the board's rescaling factor $\frac{n/2}{\sum v_i}$ is slightly less than
$1$. The rescaled vector $f_i = 1200\, A_i / S$ (with $A_i, S$ the exact
integer numerators) satisfies $\sum_i f_i = 1200 = n/2$ \emph{exactly}, and
an exact integer comparison confirms $0 \le f_i \le 1$ for all $i$ (zero
violations), so $F$ is admissible. All $2n - 1 = 4799$ lattice correlations
$\sum_i P_i Q_{i+m}$ (with $P_i = 1200 A_i$, $Q_i = S - P_i$) are exact
integers; the maximum occurs at lag $m^\ast = -92$ (i.e.
$x^\ast = -184/2400 \approx -0.0767$), and
\[
\max_{|m|\le n-1} \frac{2}{n} \sum_i f_i g_{i+m}
\;=\; \frac{2\,\max_m \sum_i P_i Q_{i+m}}{n\, S^2} \;=\; Q
\]
...
computation is exact integer arithmetic and runs in under one second, with
no floating-point operation on the certified path; see
Appendix~\ref{sec:repro} (script \texttt{erdos\_upper\_exact.py}).
\end{proof}

\begin{remark}[relation to the board value]
$\mathrm{float}(Q) = 0.3808669097979876$ agrees with the board's
floating-point score to the last bit, and the exact lag-by-lag values agree
with \texttt{numpy.correlate} to $\le 1.7\times10^{-16}$ (this also pins
down numpy's lag-indexing convention). The float agreement plays no role in
the proof; it is a sanity cross-check.
\end{remark}
...
Theorem~\ref{thm:upper} improves it by $5.994\times10^{-5}$. Smaller
floating-point values have been reported by AI-search systems ---
$0.380924\ldots$ \cite{AlphaEvolve25}, $0.380876\ldots$ \cite{TTT26},
$0.380868\ldots$ \cite{SimpleTES26} --- all of which lie \emph{above} $Q$.
...
in-progress work by others, including other EinsteinArena participants,
whose live constructions this value is built from; the construction is
Hyra's.
\end{remark}
...
Hyra step function of Section~\ref{sec:upper}, exact-cell integration gives
$c_1 = \int_{-1}^1 \cos(\pi x) F(x)\,dx \approx -0.3813$; after White's
WLOG normalization ($f \mapsto 1 - f$, then $x \mapsto -x$) the
representative configuration has
\[
\bigl(E(M),\, c_1,\, d_1\bigr) \;\approx\; (0.0003,\; 0.3813,\; \mp0.0005),
\]
so $c_1^\ast \approx 0.3813 \notin [0.33, 0.35]$. Indeed White's own
...
\cite{Haugland96,White22}), and a finite exact-integer-arithmetic evaluation
of Hyra's $n = 2400$ construction \cite{Hyra26}, independently re-runnable in
under a second. No floating-point quantity enters the certified path. This
side is new and improves \cite{Haugland16} by $5.994\times10^{-5}$.
...
The numerical bracket ``$\mu \in [0.380827,\, 0.380867]$'' suggested by the
...
\begin{itemize}
\item \texttt{scripts/erdos\_upper\_exact.py} --- exact rational evaluation.
Input: \texttt{certs/erdos\_hyra\_current.json} (Hyra's $n=2400$ vector, as
retrieved from the EinsteinArena board 2026-06-30). Run:
\texttt{python3 erdos\_upper\_exact.py}. Output must reproduce
\texttt{certs/lane\_u\_exact\_output.txt} verbatim: the exact fraction $Q$,
the $25$-digit enclosure, argmax lag $-92$, and the $[0,1]$ post-rescale
check with zero violations. Requires only the Python standard library
(\texttt{fractions}); the \texttt{numpy} cross-check at the end is optional.
\end{itemize}
...
verification harness) and an NVIDIA DGX Spark (large solves; the
...
\emph{Step-function construction for the minimum overlap problem}
($n = 2400$ vector, board score $0.3808669097979876$),
EinsteinArena, problem \texttt{erdos-min-overlap},
