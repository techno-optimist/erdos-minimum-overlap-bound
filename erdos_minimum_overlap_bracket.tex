...
(precisely $192252155 \cdot 2^{-78}$, about $2.5$ ULP at scale $256$). The
vector clears the board only because $\mathrm{float}(\sum_i f_i)$ rounds up to
exactly $256.0$; in exact arithmetic it is \emph{not} admissible. Moreover the
board's uniform rescale is here the \emph{wrong} fix: it would multiply every
entry \emph{up} by $(n/2)/\sum > 1$, and $48$ of the entries are already
saturated at the box boundary $f_i = 1$, so the rescale pushes them just above
$1$ (worst excess $\approx 2.5\times10^{-18}$), violating $f \in [0,1]$. A
uniform rescale of a sub-$(n/2)$ vector is precisely the historical
``rescale artifact'' that turns an apparent record into a strictly worse
admissible score.
...
construction, far from the maximizing lag $m^\ast = 100$). The result is exactly
...
genuine proven bound. The repair delta --- $[(0,\,192252155)]$, a $0$-based cell
...
%%RESPRINT-HEADLINE: if an independent ProjectForty2 construction certifies a
%% strictly tighter ADMISSIBLE-OUTRIGHT bound than Q_H (Theorem~\ref{thm:hyra}),
%% insert it here as the new headline theorem -- same proof engine, credit to its
%% author -- and promote its value into the abstract and Theorem~\ref{thm:main}
%% (both currently carry Q_H = 0.3808594223653146192081122). Until then the
%% headline is Hyra's Q_H.
